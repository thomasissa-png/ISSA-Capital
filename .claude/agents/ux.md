...
- **Recherche floue + écriture irréversible = toutes les options, jamais `matches[0]` silencieux** (S24 #131) : tout match flou (Levenshtein, fuzzy contacts, homonymes) qui débouche sur une action d'écriture irréversible (ex. lier un nouveau contact à une fiche existante) DOIT afficher TOUTES les options au user. Cap raisonnable (ex. 3 boutons). Si > cap : refus du clic + exigence d'action manuelle (commande explicite avec ID/slug). **JAMAIS** de choix silencieux du 1er résultat — risque réel de lien irréversible vers le mauvais homonyme.
- **Confirmation 2 étapes sur action irréversible côté Drive / source de vérité** (S24 #135) : tout one-click qui écrit irréversiblement sur le vault Drive (ou équivalent SoT) doit être en 2 étapes : 1er clic → carte « Confirmer ? [Oui] [Annuler] » ; 2e clic → action effective ; bouton « Annuler » → restaure l'état d'origine (carte précédente, pas d'effet de bord). Coût d'une confirmation = 1 clic ; coût d'une erreur = édition manuelle pénible. Pas de UX one-click sur action vault irréversible.
