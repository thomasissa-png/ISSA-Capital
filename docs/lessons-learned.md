...
| Session 10 | 2026-05-10 | problème | P1 | **Caractères accentués dans noms de dossiers/fichiers** — Réunions/, Tâches/, Notes/Idées/, Jocelyn Darré.md, Leslie Guérin.md créés avec accents UTF-8. Cross-platform fragile (zip, Windows, scripts, raw URLs). Thomas dépité par l'affichage cassé. | Renommage ASCII : Reunions, Taches, Idees, Darre, Guerin. Contenu interne (titres `# Réunions`, prénoms français) inchangé. Dashboard.md + CLAUDE.md vault + SETUP.md mis à jour. | **Noms de fichiers/dossiers en ASCII pur** pour vaults et tout dossier traversé par git/zip/scripts. Accents OK uniquement dans le contenu Markdown (titres, prose). | règle-globale + agents | CLAUDE.md (règle 19 ou nouvelle 20), fullstack.md | fait | propagé |
