...
| infrastructure (validation post-reprise) | 2026-04-07 | docs/infra/infrastructure.md + performance-audit.md + security-checklist.md + deployment-replit.md + REPLIT_ACTIONS.md | VALIDATION DES 5 LIVRABLES INFRA produits en reprise par @orchestrator dans une session précédente. Auto-évaluation gates : tous PASS sur BLOQUANT (G1 complétude, G3 handoff, G12 implémentable, G15 zéro placeholder) et REQUIS (G9 owner+action, G11 critères binaires). Architecture Next.js App Router verrouillée (7 pages + /api/contact + sitemap.ts + robots.ts), stack Resend + Plausible cookieless + fonts self-hosted conforme @legal, headers de sécurité complets (CSP scriptée autorisant uniquement plausible.io + api.resend.com), validation Zod discriminated union 3 variants, rate limit 5/IP/10min in-memory + honeypot + Turnstile optionnel, CWV cibles internes plus strictes que Google (LCP<2s, CLS<0.05, INP<100ms), Lighthouse 95+/4 axes, monitoring UptimeRobot 5min, Deployment Replit Autoscale, rollback documenté, REPLIT_ACTIONS.md exhaustif (initial + récurrent + urgence). Mise à jour historique agents (étape oubliée par la session précédente). | Validation complète plutôt que réécriture — les 5 fichiers couvrent 100% du scope avec un niveau de détail conforme au standard @infrastructure. Re-générer aurait été du churn sans valeur ajoutée. Choix techniques validés : (1) Rate limit in-memory vs Upstash — suffisant pour trafic vitrine, migration vers Upstash documentée si pic, (2) UptimeRobot vs BetterStack — gratuit et adapté à un site sans SLA contractuel, (3) Pas de Sentry — overkill pour un site statique, console.error structuré + logs Replit suffisent, (4) Deployment Autoscale vs Reserved VM — coût optimisé pour faible trafic prévu, scale auto en cas de pic, (5) CSP avec 'unsafe-inline' pour styles : nécessaire pour Next.js App Router (CSS critique inline), trade-off accepté car pas de XSS stocké possible (formulaires écrivent dans email Resend, pas en DB), (6) AI crawlers explicitement autorisés (GPTBot/ClaudeBot/PerplexityBot) : aligné avec stratégie GEO et Principe directeur #0 (présence institutionnelle dans les réponses IA = vitrine elle-même). |
