...
15. **Bloc "Sources amont" obligatoire en tête de chaque livrable.** Chaque livrable produit dans `docs/` DOIT commencer par un bloc `> Sources amont :` listant les fichiers `docs/...` lus avant production. Ce bloc précède le titre H1 du livrable et permet à @reviewer de vérifier G7 (cohérence avec amont) en un seul Read. Format : `> Sources amont : docs/strategy/brand-platform.md, docs/product/functional-specs.md, docs/copy/brand-voice.md`. Si aucune source amont n'est lue (rare — uniquement Phase 0 initiale), écrire `> Sources amont : aucune (Phase 0 — première production)`. Source : learning ISSA Capital session 4 (P2 — pattern observé sur 13 livrables session 4, élimination 100% des contradictions G7).
16. **Volume biographique calibré sur la valeur pour le persona (projets vitrine).** Pour tout projet de type vitrine / family office / holding familiale / société patrimoniale, le volume de texte biographique d'un fondateur doit être calibré sur la valeur pour le persona, **pas sur la disponibilité des faits**. Un fondateur qui a fait 5 étapes marquantes n'a pas besoin de 5 paragraphes — 1 phrase pivot suffit si elle est juste. Règles : bio fondateur maximum 4 phrases en version RICHE, 2-3 phrases en version INT, 1 phrase en version MIN. Couper toute étape qui n'apporte rien au persona même si elle est "vraie". Test de coupe : "si je retire cette phrase, est-ce que le persona comprend encore pourquoi ce fondateur est crédible pour ce projet ?" Si oui → couper. Source : learning ISSA Capital session 6 (P2 — insistance Thomas "trop de famille", "toute la vie de Jean-Pierre et Thomas ne paraissent pas si importantes", "site vitrine épurée pas musée biographique").
17. **Chiffres narratifs vs chiffres factuels (projets vitrine).** Les chiffres doivent **raconter, pas prouver**. Un pourcentage juridique brut (50%, 30%, 20%) n'a pas sa place dans une vitrine — il est utile en due diligence, pas en landing. OUI aux stats narratives (année de fondation, nombre de participations, nombre de générations, horizon temporel). NON aux pourcentages juridiques bruts sans contexte narratif dans les livrables client-facing. Règle de substitution : si un chiffre semble nécessaire mais est un pourcentage juridique, chercher une alternative narrative. Source : learning ISSA Capital session 6 (P3 — stat "50%" détention Gradient One refusée par Thomas, remplacée par "2020 Co-fondation / 6 Participations / 3 Générations").
18. **Zéro credential en clair dans les fichiers committés.** Ne JAMAIS committer de clé API, token, mot de passe ou secret dans un fichier `.md`, `.ts`, `.js`, `.json` ou tout autre fichier versionné — même dans un fichier "brief", "documentation" ou "guide de déploiement". Utiliser `.env.local` (gitignored) dès la première mention et référencer la variable par `[CLÉ DANS .env.local → NOM_VARIABLE]` dans la documentation. Si une clé a fuité dans l'historique git → rotation obligatoire sauf décision explicite du fondateur. Source : learning ISSA Capital session 7-8 (P1 — clé Craft `pdk_9b7b...` en clair dans un brief .md committé depuis session 4, détectée et purgée session 8).
19. **Connectors natifs Claude AVANT tout setup MCP technique.** Avant de proposer un guide d'installation MCP custom (Node.js, package npm, PAT, config JSON dans `claude_desktop_config.json`), VÉRIFIER OBLIGATOIREMENT via WebFetch sur `claude.com/connectors` si l'outil cible est dans le directory connectors natifs Claude (50+ outils en 1 clic OAuth). Si oui → guide minimaliste pointant vers `claude.ai/customize/connectors`, point. Source : learning ISSA Capital session 9 P2 (Gmail/Calendar) + récidive session 10 P1 (Asana + Craft) — perte de temps Thomas répétée.
20. **Noms de fichiers/dossiers en ASCII pur.** Pour tout vault, dossier traversé par git/zip/scripts, ou structure cross-plateforme : utiliser uniquement des caractères ASCII dans les noms de fichiers et dossiers (pas d'accents, pas d'emojis, pas de caractères spéciaux). Les accents UTF-8 (é, è, à, ç) sont autorisés UNIQUEMENT dans le contenu Markdown (titres, prose, frontmatter values). Exemples conformes : `Reunions/`, `Taches/`, `Notes/Idees/`, `Jocelyn Darre.md`. Exemples non conformes : `Réunions/`, `Tâches/`, `Notes/Idées/`, `Jocelyn Darré.md`. Raison : zip/Windows/scripts/raw URLs cassent fréquemment l'encoding UTF-8 dans les chemins. Source : learning ISSA Capital session 10 P1.
21. **Migration OAuth scope : deployer AVANT de re-authentifier.** Pour toute migration de scope OAuth (ex : `drive.file` vers `drive`) : (a) deployer le code qui demande le nouveau scope AVANT que l'utilisateur revoque/re-OAuth, (b) ajouter sur la page de retour OAuth un appel `tokeninfo` qui affiche le scope effectif recu (vert si attendu, rouge sinon), (c) ne jamais faire confiance a un test indirect (un upload qui reussit ne prouve pas que le scope elargi est actif). Source : learning ISSA Capital session 11 P1.
22. **Logs de diagnostic : `console.warn` minimum sur Replit/Vercel/Netlify.** Pour tout log de diagnostic destine a etre lu par l'utilisateur en production, utiliser `console.warn` au minimum. `console.log` est INFO-level et masque par les UIs de logs en filtrage par defaut (Replit Logs ne montre que WARN/ERROR par defaut). Source : learning ISSA Capital session 11 P2.
23. **APIs a resultat silencieusement vide : liste puis filtre local.** Pour toute API qui peut retourner silencieusement un resultat vide selon le scope/permissions (Google Drive, etc.) : preferer "liste tous les enfants + filtre cote code" plutot que "query stricte cote API". Logger la liste visible en mode WARN pour diagnostic immediat si scope insuffisant. Source : learning ISSA Capital session 11 P1.
24. **Dispatch webhook media : par MIME exhaustif, pas par type de message.** Pour tout webhook qui recoit des medias (Telegram, Slack, Discord, WhatsApp...) : le mode "Send as file" arrive systematiquement en `message.document` avec `mime_type` reflechissant le contenu reel (`image/*`, `video/*`, `audio/*`). Ne JAMAIS supposer qu'un type est associe a un seul payload. Regles : (a) router par MIME du document AVANT de tester le type de message, (b) lister tous les types correspondants au cas d'usage en debut de spec workflow media (photos = `message.photo` + `message.document` mime `image/*` ; videos = `message.video` + `message.document` mime `video/*`), (c) demander explicitement au fondateur en debut de spec "envoies-tu parfois en mode fichier ? videos ? audios ?". Source : learning ISSA Capital session 12 P1 (bugs photos `c15ed66` + gap videos `15ccc8d` — photos en mode fichier tombaient dans Documents au lieu de Photos, videos non gerees du tout).
