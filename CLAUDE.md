...
- **R5 (P0 #99, S15)** — **Édition fichier Drive existant = PATCH in-place via `_zap_raw_request`** (`/upload/drive/v3/files/{fileId}?uploadType=media`). JAMAIS create+delete : casse fileId, wikilinks Obsidian, partages. Voir `docs/drive-edit-strategy.md`.
